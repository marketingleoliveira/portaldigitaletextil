...
const MonthlyActivityRankingCard: React.FC = () => {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('monthly-activity-ranking-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_activity_sessions' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          sessionsRef.current = [...sessionsRef.current, payload.new];
        } else if (payload.eventType === 'UPDATE') {
          sessionsRef.current = sessionsRef.current.map(s => s.id === payload.new.id ? payload.new : s);
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'user_presence' }, (payload: any) => {
        ...
        if (newData?.is_online) {
          ...
          if (Date.now() - lastSeen < 30000) onlineUsersRef.current.add(newData.user_id);
          else onlineUsersRef.current.delete(newData.user_id);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (profilesRef.current.length > 0) calculateRanking();
    }, 1000);
  ...
  const fetchData = async () => {
    try {
      ...
      const [profilesRes, rolesRes, sessionsRes, presenceRes] = await Promise.all([
        supabase.from('profiles').select('id, full_name, avatar_url, region').eq('is_active', true),
      ...
      const eligibleUserIds = new Set((rolesRes.data || []).filter(r => r.role !== 'dev').map(r => r.user_id));
      const roleMap = new Map((rolesRes.data || []).map(r => [r.user_id, r.role]));

      if (profilesRes.data) {
        profilesRef.current = profilesRes.data
          .filter(p => eligibleUserIds.has(p.id))
          .map(p => ({ ...p, role: roleMap.get(p.id) as AppRole }));
      }
      if (sessionsRes.data) sessionsRef.current = sessionsRes.data;
      if (presenceRes.data) onlineUsersRef.current = new Set(presenceRes.data.map(p => p.user_id));
  ...
  const calculateRanking = () => {
    ...
    sessionsRef.current.forEach(session => {
      if (!profilesRef.current.some(p => p.id === session.user_id)) return;
      ...
      userDurations.set(session.user_id, (userDurations.get(session.user_id) || 0) + duration);
    });
  ...
  const getMedalStyle = (position: number) => {
    switch (position) {
      case 0: return { bg: 'bg-gradient-to-r from-yellow-400/20 via-yellow-300/20 to-yellow-500/20', border: 'border-yellow-400/50', medal: '🏆', shimmer: true };
      case 1: return { bg: 'bg-gradient-to-r from-slate-300/20 via-slate-200/20 to-slate-400/20', border: 'border-slate-400/50', medal: '🥈', shimmer: false };
      case 2: return { bg: 'bg-gradient-to-r from-amber-600/20 via-amber-500/20 to-amber-700/20', border: 'border-amber-600/50', medal: '🥉', shimmer: false };
      default: return { bg: 'bg-muted/50', border: 'border-transparent', medal: '', shimmer: false };
    }
  };
        ...
        {ranking.length === 0 ? (
          ...
          ranking.map((user, index) => {
            ...
            return (
              <div
                key={user.user_id}
                className={`relative flex items-center gap-3 p-3 rounded-lg border ${style.bg} ${style.border} transition-all ${style.shimmer ? 'animate-pulse' : ''}`}
              >
...
export default MonthlyActivityRankingCard;
