...
import { Wallet, Search, Loader2, ExternalLink, CheckCircle2, XCircle, Clock, BadgeDollarSign, FileDown, Trash2, Merge, Plus, } from 'lucide-react';
...
const FinanceiroReembolsos = () => {
    ...
    const deleteReport = async (r) => {
        ...
        try {
            ...
            const paths = its.map(i => i.receipt_path).filter(Boolean);
            if (paths.length) {
    ...
    const mergeSelected = async () => {
        ...
        if (ids.length < 2) {
            toast.error('Selecione pelo menos 2 reembolsos');
            return;
        }
        const chosen = reports.filter(r => ids.includes(r.id));
        const userIds = new Set(chosen.map(r => r.user_id));
        if (userIds.size > 1) {
            toast.error('Só é possível agrupar reembolsos do mesmo colaborador');
            return;
        }
        setMerging(true);
    ...
    const updateStatus = async (id, status) => {
        ...
        if (detailReport?.id === id)
            setDetailReport({ ...detailReport, status });
    };
    const exportReportPdf = async (report, includeReceipts = true) => {
        ...
        try {
            ...
            // Word wrap helper
            const wrap = (text, maxWidth, size, f) => {
    ...
    return (<DashboardLayout>
      <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {['pendente', 'aprovado', 'rejeitado', 'pago'].map(s => {
        ...
        <Card>
          <CardContent className="pt-6 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar por usuário, e-mail, título ou destino..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9"/>
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between gap-3">
              ...
              {isDev && selected.size >= 2 && (<AlertDialog>
          ...
          <CardContent className="p-0">
            {loading ? (<div className="flex justify-center py-12">
              ...
              </div>) : filtered.length === 0 ? (<p className="text-center text-sm text-muted-foreground py-12">
                ...
                return (<TableRow key={r.id} className="cursor-pointer" onClick={() => setDetailReport(r)}>
                          {isDev && (<TableCell onClick={(e) => e.stopPropagation()} className="w-8">
                          ...
                          <TableCell>
                            ...
                            {r.trip_destination && (<p className="text-xs text-muted-foreground">{r.trip_destination}</p>)}
                          </TableCell>
                          ...
                          <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                            <div className="flex items-center justify-end gap-1">
                              ...
                              {isDev && (<Button size="sm" variant="ghost" className="h-8 px-2" title="Adicionar comprovante" onClick={() => openAddItem(r)}>
      ...
      {/* Detail dialog */}
      <Dialog open={!!detailReport} onOpenChange={(o) => { if (!o)
          ...
          {detailReport && (<>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="rounded-lg border">
                    <Table>
                      ...
                      <TableBody>
                        {(items[detailReport.id] || []).map(it => (<TableRow key={it.id}>
                            ...
                            <TableCell className="text-right">
                              {it.receipt_url ? (<a href={it.receipt_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline">
                ...
                <div className="flex flex-wrap gap-2 pt-2 border-t">
                  ...
                  <Button variant="outline" size="sm" onClick={() => updateStatus(detailReport.id, 'pendente')} disabled={detailReport.status === 'pendente'}>
                    <Clock className="w-4 h-4 mr-1"/> Pendente
                  </Button>
                  <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" onClick={() => updateStatus(detailReport.id, 'aprovado')} disabled={detailReport.status === 'aprovado'}>
                    <CheckCircle2 className="w-4 h-4 mr-1"/> Aprovar
                  </Button>
                  <Button size="sm" variant="destructive" onClick={() => updateStatus(detailReport.id, 'rejeitado')} disabled={detailReport.status === 'rejeitado'}>
                    <XCircle className="w-4 h-4 mr-1"/> Rejeitar
                  </Button>
                  <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => updateStatus(detailReport.id, 'pago')} disabled={detailReport.status === 'pago'}>
      ...
      <Dialog open={!!addItemFor} onOpenChange={(o) => { if (!o)
        setAddItemFor(null); }}>
